...
class Spending: FirestoreIdentifiable {
  ...
  init(name: String, amount: Double, date: Date, spendingType: SpendingType?, created: Date, projectType: ProjectInfo? = nil, accountType: DAccountType? = nil, isArchived: Bool = false, currencyCode: String? = nil) {
    self.name = name
    self.amount = amount
    self.date = date
